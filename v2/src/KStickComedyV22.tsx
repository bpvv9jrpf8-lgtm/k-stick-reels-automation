...
const Plate: React.FC = () => (
  <div style={{position: 'absolute', left: 540, top: 1095, width: 210, height: 68, borderRadius: '50%', transform: 'translate(-50%,-50%)', background: '#fafafa', border: '8px solid #bcbcbc', boxShadow: '0 9px 10px rgba(0,0,0,.16)', zIndex: 17}} />
);
...
const Pizza: React.FC<{x: number; y: number; scale: number; rotation: number; opacity: number; bite: number}> = ({x, y, scale, rotation, opacity, bite}) => (
  <div style={{position: 'absolute', left: x, top: y, width: 160, height: 160, transform: `translate(-50%,-50%) rotate(${rotation}deg) scale(${scale})`, transformOrigin: '50% 50%', zIndex: 35, opacity, filter: 'drop-shadow(0 11px 8px rgba(0,0,0,.20))'}}>
    <svg viewBox="0 0 160 160" width="160" height="160">
      <defs>
        <mask id="pizza-mask-v22">
          <rect width="160" height="160" fill="white" />
          {bite > 0.2 && <circle cx="112" cy="88" r="22" fill="black" />}
          {bite > 0.55 && <circle cx="96" cy="119" r="20" fill="black" />}
        </mask>
      </defs>
      <g mask="url(#pizza-mask-v22)">
  ...
  return (
    <>
      {[0, 1, 2, 3, 4].map((i) => {
        const a = -2.2 + i * 0.55;
        const d = 25 + p * (40 + i * 7);
        return <div key={i} style={{position: 'absolute', left: x + Math.cos(a) * d, top: y + Math.sin(a) * d, width: 9 + i * 2, height: 9 + i * 2, borderRadius: '50%', background: i % 2 ? '#ffd35a' : '#d73535', opacity: 1 - p * 0.5, zIndex: 44}} />;
      })}
    </>
  );
};
...
const camera = (frame: number) => {
  if (frame < sec(1.3)) return {scale: 1.24, x: 0, y: -105};
  if (frame < sec(3.5)) return {scale: 1.0, x: 0, y: 0};
  if (frame < sec(5.6)) return {scale: 1.03, x: 0, y: -10};
  if (frame < sec(9.0)) return {scale: 1.08, x: 0, y: -38};
  if (frame < sec(13.2)) return {scale: 1.10, x: 0, y: -28};
  if (frame < sec(14.05)) return {scale: 1.18, x: 0, y: -90};
  if (frame < sec(17.6)) return {scale: 1.0, x: 0, y: 0};
  if (frame < sec(20.1)) return {scale: 1.15, x: 78, y: -32};
  return {scale: 1.0, x: 0, y: 0};
};

export const KStickComedyV22: React.FC = () => {
  ...
  const kX = interpolate(frame, [0, sec(0.7)], [-250, 50], clamp);
  const zX = interpolate(frame, [sec(0.25), sec(1.05)], [1110, 720], clamp);
  ...
  const pizzaX = interpolate(frame, [0, sec(10.45), sec(11.25), sec(11.75), sec(12.2), sec(12.65), sec(13.05), sec(13.45)], [540, 540, handX, mouthX + 25, handX, mouthX + 25, handX, mouthX + 20], clamp);
  const pizzaY = interpolate(frame, [0, sec(10.45), sec(11.25), sec(11.75), sec(12.2), sec(12.65), sec(13.05), sec(13.45)], [1058, 1058, handY + 22, mouthY + 8, handY + 30, mouthY + 8, handY + 34, mouthY + 8], clamp);
  const pizzaScale = interpolate(frame, [0, sec(10.45), sec(11.25), sec(11.75), sec(12.2), sec(12.65), sec(13.05), sec(13.45)], [1, 1, 0.60, 0.48, 0.48, 0.38, 0.30, 0.12], clamp);
  const pizzaOpacity = interpolate(frame, [sec(13.12), sec(13.48)], [1, 0], clamp);
  const pizzaRotation = interpolate(frame, [sec(10.45), sec(13.45)], [0, 25], clamp);
  ...
  const cutFlash = Math.max(...cutTimes.map((t) => interpolate(Math.abs(frame - sec(t)), [0, 2], [0.13, 0], clamp)));
  ...
  return (
    <AbsoluteFill style={{background: '#fff4df', overflow: 'hidden'}}>
      <div style={{position: 'absolute', inset: 0, transform: `translate(${cam.x + shake}px,${cam.y}px) scale(${cam.scale})`, transformOrigin: '50% 58%'}}>
        ...
        <Crumbs x={mouthX + 10} y={mouthY + 8} p={crumbP} />

        {frame >= sec(13.25) && frame < sec(14.25) && (
          <div style={{position: 'absolute', left: 540, top: 1005, transform: `translateX(-50%) scale(${0.65 + reveal * 0.35})`, zIndex: 85, fontFamily: 'Arial Black, Arial, sans-serif', fontSize: 56, fontWeight: 900, color: '#fff', WebkitTextStroke: '7px #111', paintOrder: 'stroke'}}>EMPTY?!</div>
        )}
